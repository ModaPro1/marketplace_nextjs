        ...
        return (
          <div key={item.id} className="flex gap-5 mb-3 items-start">
            ...
            <div className="flex-1">
              ...
              {option && (
                <div className="py-1 px-2 border border-gray-500 text-xs rounded-md w-fit mt-2 mb-4">{option.name}</div>
              )}
              <div className="flex justify-between mt-3 items-center">
                <ItemQuantity quantity={item.quantity} onChange={(quantity) => changeItemQuantity(item.id, quantity)} />
